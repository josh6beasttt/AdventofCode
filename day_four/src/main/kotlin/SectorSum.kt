...
object SectorSum {
    val ROOMS = """gbc-frperg-pubpbyngr-znantrzrag-377[rgbnp]
...
jyfvnlupj-ibuuf-svnpzapjz-851[gmsnf]"""
    ...
    @JvmStatic fun main(args: Array<String>) {
        println(getResult(ROOMS.split("\n")))
    }

    fun getResult(rooms: List<String>): Int {
        var total: Int = 0

        for (roomString in rooms) {
            var room = roomString.toRoom()
            if(checkSum(room)) total += room.number
    ...
    private fun checkSum(room: Room): Boolean {
        var letterMap = mutableMapOf<Char, Int>()
        var letterList = mutableListOf<LetterCount>()
        ...
        for ((key, value) in letterMap) {
            letterList.add(LetterCount(key.toString(), value))
        }
        ...
        var value: String = ""
        for (i in 0..4) {
            value += letterList[i].letter
        }

        return value.equals(room.checksum)
    }

    private fun String.toRoom(): Room {
        val (instructions, checksum) = this.split("[")
        val dashIndex: Int = (instructions.length - 1 downTo 0).firstOrNull { instructions[it].equals('-') } ?: 0

        val letters = instructions.substring(0, dashIndex).replace("-", "")
        val number = instructions.substring(dashIndex + 1, instructions.length).toInt()

        return Room(letters, number, checksum.replace("]", ""))
    }

    data class Room(var letters: String, val number: Int, val checksum: String)

    data class LetterCount(val letter: String, var count: Int) : Comparable<LetterCount> {
        override fun compareTo(other: LetterCount): Int {
            var i = this.count.compareTo(other.count)
            if (i != 0) return -i
